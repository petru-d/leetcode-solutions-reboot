...
namespace p190
{
    class Solution
    {
      public:
        ...
        {
            constexpr std::array<uint8_t, 256> reverser{
            ...
            n >>= 8;
            uint8_t byte1 = n & 0xff;

            n >>= 8;
            uint8_t byte2 = n & 0xff;

            n >>= 8;
            uint8_t byte3 = n & 0xff;

            return (reverser[byte0] << 24) | (reverser[byte1] << 16) | (reverser[byte2] << 8) | (reverser[byte3]);
        }

        void print_reverser()
        {
            std::ofstream of("C:/work/bitz.txt");
            of << "{";
