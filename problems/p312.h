    ...
    class Solution
    {
      public:
        int maxCoins(std::vector<int>& nums)
        {
            ...
            std::vector<std::vector<int>> rangeValues(nums.size(), std::vector<int>(nums.size(), 0));
            ...
            for (int len = 1; len <= N; ++len)
            {
                for (int start = 1; start <= N - len + 1; ++start)
                {
                    ...
                    {
                        int coins = rangeValues[start][size_t(final) - 1] + rangeValues[size_t(final) + 1][end]; // coins from popping subranges
                        coins += nums[size_t(start) - 1] * nums[final] * nums[size_t(end) + 1];                  // coins from final pop
                        if (coins > bestCoins)
                            bestCoins = coins;
                    }
